# Signal-Server HD run: `CD4JPX-L`

Manifest generated by `scripts/runsignal-hd-transparent.sh`.

## Coverage preview

![Coverage map: CD4JPX-L](./CD4JPX-L.png)
...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CD4JPX-L`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CD4JPX-L -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.7333\, -lon -72.95 -txh 25 -f 145.43 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.7333\, -lon -72.95 -txh 25 -f 145.43 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CD4JPX-L/CD4JPX-L 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CD4JPX-L/CD4JPX-L.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CD4JPX-L/CD4JPX-L.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CD4JPX-L/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CD4JPX-L && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CD4JPX-L /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CD4JPX-L.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CD4JPX-L.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CD4JPX-L.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CD4JPX-L.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CD4JPX-L.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
