...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3SER-UHF`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE3SER-UHF -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.45\, -lon -70.6667 -txh 25 -f 433.52 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.45\, -lon -70.6667 -txh 25 -f 433.52 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3SER-UHF/CE3SER-UHF 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3SER-UHF/CE3SER-UHF.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3SER-UHF/CE3SER-UHF.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE3SER-UHF/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE3SER-UHF && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3SER-UHF /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE3SER-UHF.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE3SER-UHF.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE3SER-UHF.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE3SER-UHF.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE3SER-UHF.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
