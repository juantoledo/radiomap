...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6ROX-L`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE6ROX-L -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.5292\, -lon -72.435 -txh 25 -f 146.8 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.5292\, -lon -72.435 -txh 25 -f 146.8 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6ROX-L/CE6ROX-L 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6ROX-L/CE6ROX-L.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6ROX-L/CE6ROX-L.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE6ROX-L/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE6ROX-L && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6ROX-L /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE6ROX-L.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE6ROX-L.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE6ROX-L.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE6ROX-L.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE6ROX-L.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
