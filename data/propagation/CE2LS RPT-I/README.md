...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2LS RPT-I`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE2LS\ RPT-I -dbg -lid data/SRTMGL3.asc -resample 2 -lat -30.0472\, -lon -71.3575 -txh 25 -f 146.22 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 25 -txg 7 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -30.0472\, -lon -71.3575 -txh 25 -f 146.22 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 25 -txg 7 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2LS\ RPT-I/CE2LS\ RPT-I 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2LS\ RPT-I/CE2LS\ RPT-I.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2LS\ RPT-I/CE2LS\ RPT-I.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE2LS RPT-I/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE2LS\ RPT-I && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2LS\ RPT-I /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE2LS RPT-I.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE2LS RPT-I.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE2LS RPT-I.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE2LS RPT-I.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE2LS RPT-I.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
