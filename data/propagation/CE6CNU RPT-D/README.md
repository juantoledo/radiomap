...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6CNU RPT-D`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE6CNU\ RPT-D -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.8386\, -lon -72.8547 -txh 25 -f 147.93 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 25 -txg 6 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.8386\, -lon -72.8547 -txh 25 -f 147.93 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 25 -txg 6 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6CNU\ RPT-D/CE6CNU\ RPT-D 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6CNU\ RPT-D/CE6CNU\ RPT-D.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6CNU\ RPT-D/CE6CNU\ RPT-D.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE6CNU RPT-D/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE6CNU\ RPT-D && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6CNU\ RPT-D /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE6CNU RPT-D.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE6CNU RPT-D.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE6CNU RPT-D.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE6CNU RPT-D.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE6CNU RPT-D.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
