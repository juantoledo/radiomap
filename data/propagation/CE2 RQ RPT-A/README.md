...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2 RQ RPT-A`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE2\ RQ\ RPT-A -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.4114\, -lon -71.6819 -txh 25 -f 146.34 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 5 -txg 3 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.4114\, -lon -71.6819 -txh 25 -f 146.34 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 5 -txg 3 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2\ RQ\ RPT-A/CE2\ RQ\ RPT-A 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2\ RQ\ RPT-A/CE2\ RQ\ RPT-A.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2\ RQ\ RPT-A/CE2\ RQ\ RPT-A.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE2 RQ RPT-A/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE2\ RQ\ RPT-A && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE2\ RQ\ RPT-A /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE2 RQ RPT-A.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE2 RQ RPT-A.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE2 RQ RPT-A.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE2 RQ RPT-A.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE2 RQ RPT-A.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
