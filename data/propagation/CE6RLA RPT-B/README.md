...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6RLA RPT-B`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE6RLA\ RPT-B -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.7958\, -lon -72.5472 -txh 25 -f 438.4 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 7 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -38.7958\, -lon -72.5472 -txh 25 -f 438.4 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 7 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6RLA\ RPT-B/CE6RLA\ RPT-B 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6RLA\ RPT-B/CE6RLA\ RPT-B.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6RLA\ RPT-B/CE6RLA\ RPT-B.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE6RLA RPT-B/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE6RLA\ RPT-B && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE6RLA\ RPT-B /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE6RLA RPT-B.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE6RLA RPT-B.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE6RLA RPT-B.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE6RLA RPT-B.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE6RLA RPT-B.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
