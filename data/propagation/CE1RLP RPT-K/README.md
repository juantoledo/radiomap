...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1RLP RPT-K`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE1RLP\ RPT-K -dbg -lid data/SRTMGL3.asc -resample 2 -lat -22.6945\, -lon -68.4003 -txh 25 -f 147.75 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 15 -txg 6.7 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -22.6945\, -lon -68.4003 -txh 25 -f 147.75 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 15 -txg 6.7 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1RLP\ RPT-K/CE1RLP\ RPT-K 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1RLP\ RPT-K/CE1RLP\ RPT-K.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1RLP\ RPT-K/CE1RLP\ RPT-K.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE1RLP RPT-K/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE1RLP\ RPT-K && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1RLP\ RPT-K /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE1RLP RPT-K.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE1RLP RPT-K.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE1RLP RPT-K.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE1RLP RPT-K.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE1RLP RPT-K.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
