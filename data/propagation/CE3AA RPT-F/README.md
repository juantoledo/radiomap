...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA RPT-F`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE3AA\ RPT-F -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.1842\, -lon -70.9772 -txh 25 -f 438.375 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 6 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.1842\, -lon -70.9772 -txh 25 -f 438.375 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 6 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-F/CE3AA\ RPT-F 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-F/CE3AA\ RPT-F.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-F/CE3AA\ RPT-F.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE3AA RPT-F/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE3AA\ RPT-F && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-F /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE3AA RPT-F.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE3AA RPT-F.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE3AA RPT-F.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE3AA RPT-F.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE3AA RPT-F.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
