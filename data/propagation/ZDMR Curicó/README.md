...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR Curicó`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation ZDMR\ Curicó -dbg -lid data/SRTMGL3.asc -resample 2 -lat -34.9828\, -lon -71.2394 -txh 25 -f 146.58 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -34.9828\, -lon -71.2394 -txh 25 -f 146.58 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Curicó/ZDMR\ Curicó 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Curicó/ZDMR\ Curicó.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Curicó/ZDMR\ Curicó.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/ZDMR Curicó/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/ZDMR\ Curicó && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Curicó /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `ZDMR Curicó.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `ZDMR Curicó.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `ZDMR Curicó.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `ZDMR Curicó.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `ZDMR Curicó.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
