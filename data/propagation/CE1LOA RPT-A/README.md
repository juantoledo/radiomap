...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1LOA RPT-A`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE1LOA\ RPT-A -dbg -lid data/SRTMGL3.asc -resample 2 -lat -22.7583\, -lon -68.9911 -txh 25 -f 146.1 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 10 -txg 6 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -22.7583\, -lon -68.9911 -txh 25 -f 146.1 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 10 -txg 6 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1LOA\ RPT-A/CE1LOA\ RPT-A 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1LOA\ RPT-A/CE1LOA\ RPT-A.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1LOA\ RPT-A/CE1LOA\ RPT-A.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE1LOA RPT-A/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE1LOA\ RPT-A && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1LOA\ RPT-A /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE1LOA RPT-A.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE1LOA RPT-A.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE1LOA RPT-A.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE1LOA RPT-A.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE1LOA RPT-A.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
