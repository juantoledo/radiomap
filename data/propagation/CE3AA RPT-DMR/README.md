...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA RPT-DMR`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE3AA\ RPT-DMR -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.1842\, -lon -70.9772 -txh 25 -f 146.04 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 10 -txg 6 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -33.1842\, -lon -70.9772 -txh 25 -f 146.04 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -txp 10 -txg 6 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-DMR/CE3AA\ RPT-DMR 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-DMR/CE3AA\ RPT-DMR.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-DMR/CE3AA\ RPT-DMR.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE3AA RPT-DMR/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE3AA\ RPT-DMR && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3AA\ RPT-DMR /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE3AA RPT-DMR.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE3AA RPT-DMR.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE3AA RPT-DMR.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE3AA RPT-DMR.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE3AA RPT-DMR.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
