...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CA1HDG-R`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CA1HDG-R -dbg -lid data/SRTMGL3.asc -resample 2 -lat -29.95\, -lon -71.3333 -txh 25 -f 146.42 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -29.95\, -lon -71.3333 -txh 25 -f 146.42 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CA1HDG-R/CA1HDG-R 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CA1HDG-R/CA1HDG-R.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CA1HDG-R/CA1HDG-R.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CA1HDG-R/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CA1HDG-R && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CA1HDG-R /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CA1HDG-R.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CA1HDG-R.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CA1HDG-R.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CA1HDG-R.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CA1HDG-R.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
