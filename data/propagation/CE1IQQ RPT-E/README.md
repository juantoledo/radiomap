...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1IQQ RPT-E`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE1IQQ\ RPT-E -dbg -lid data/SRTMGL3.asc -resample 2 -lat -20.3333\, -lon -70.1 -txh 25 -f 438.475 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 7.5 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -20.3333\, -lon -70.1 -txh 25 -f 438.475 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 7.5 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1IQQ\ RPT-E/CE1IQQ\ RPT-E 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1IQQ\ RPT-E/CE1IQQ\ RPT-E.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1IQQ\ RPT-E/CE1IQQ\ RPT-E.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE1IQQ RPT-E/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE1IQQ\ RPT-E && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE1IQQ\ RPT-E /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE1IQQ RPT-E.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE1IQQ RPT-E.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE1IQQ RPT-E.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE1IQQ RPT-E.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE1IQQ RPT-E.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
