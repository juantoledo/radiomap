...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE4RSF RPT-C`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE4RSF\ RPT-C -dbg -lid data/SRTMGL3.asc -resample 2 -lat -34.5453\, -lon -70.8017 -txh 25 -f 439.98 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 3 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -34.5453\, -lon -70.8017 -txh 25 -f 439.98 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 3 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE4RSF\ RPT-C/CE4RSF\ RPT-C 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE4RSF\ RPT-C/CE4RSF\ RPT-C.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE4RSF\ RPT-C/CE4RSF\ RPT-C.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE4RSF RPT-C/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE4RSF\ RPT-C && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE4RSF\ RPT-C /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE4RSF RPT-C.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE4RSF RPT-C.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE4RSF RPT-C.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE4RSF RPT-C.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE4RSF RPT-C.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
