...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3BIO`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE3BIO -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.4697\, -lon -72.3537 -txh 25 -f 146.19 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.4697\, -lon -72.3537 -txh 25 -f 146.19 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3BIO/CE3BIO 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3BIO/CE3BIO.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3BIO/CE3BIO.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE3BIO/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE3BIO && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE3BIO /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE3BIO.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE3BIO.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE3BIO.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE3BIO.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE3BIO.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
