...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE5RCU RPT-C`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation CE5RCU\ RPT-C -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.7532\, -lon -73.3617 -txh 25 -f 438.45 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 6 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -37.7532\, -lon -73.3617 -txh 25 -f 438.45 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_70cm -rt -110 -txp 10 -txg 6 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE5RCU\ RPT-C/CE5RCU\ RPT-C 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE5RCU\ RPT-C/CE5RCU\ RPT-C.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE5RCU\ RPT-C/CE5RCU\ RPT-C.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/CE5RCU RPT-C/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/CE5RCU\ RPT-C && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/CE5RCU\ RPT-C /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `CE5RCU RPT-C.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `CE5RCU RPT-C.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `CE5RCU RPT-C.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `CE5RCU RPT-C.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `CE5RCU RPT-C.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
