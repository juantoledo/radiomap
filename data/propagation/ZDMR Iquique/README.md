...
- **Run output directory:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR Iquique`
- **`OUTPUT_DIR`:** `/home/jtoledoc/dev/Signal-Server2/Signal-Server/output`

### Wrapper invocation

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/scripts/runsignal-hd-transparent.sh --copy-to /home/jtoledoc/dev/radiomap/data/propagation ZDMR\ Iquique -dbg -lid data/SRTMGL3.asc -resample 2 -lat -20.2144\, -lon -70.1525 -txh 25 -f 145.78 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 
```

### signalserverHD (final `-o` is appended by the wrapper)

```bash
/home/jtoledoc/dev/Signal-Server2/Signal-Server/build/signalserverHD -dbg -lid data/SRTMGL3.asc -resample 2 -lat -20.2144\, -lon -70.1525 -txh 25 -f 145.78 -m -R 200 -pm 1 -dbm -ant /home/jtoledoc/dev/Signal-Server2/Signal-Server/antenna/generic_omni_2m -rt -110 -erp 20.0 -o /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Iquique/ZDMR\ Iquique 
```

### PNG conversion (white → transparent)

```bash
convert /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Iquique/ZDMR\ Iquique.ppm -transparent white -channel Alpha PNG32:/home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Iquique/ZDMR\ Iquique.png 
```

### Copy (`--copy-to`)

- **Destination parent:** `/home/jtoledoc/dev/radiomap/data/propagation`
- **Copied path:** `/home/jtoledoc/dev/radiomap/data/propagation/ZDMR Iquique/`

```bash
mkdir -p /home/jtoledoc/dev/radiomap/data/propagation && rm -rf /home/jtoledoc/dev/radiomap/data/propagation/ZDMR\ Iquique && cp -a /home/jtoledoc/dev/Signal-Server2/Signal-Server/output/ZDMR\ Iquique /home/jtoledoc/dev/radiomap/data/propagation/
```
...
| File | Description |
|------|-------------|
| `ZDMR Iquique.dcf` | dBm color palette: threshold lines and RGB colors used when plotting with `-dbm`. |
| `ZDMR Iquique.pgw` | World file (Esri): pixel size and map origin to georeference the raster in WGS-84. |
| `ZDMR Iquique.png` | PNG derived from the PPM via ImageMagick; white pixels set transparent for map overlays. |
| `ZDMR Iquique.ppm` | Portable pixmap from Signal-Server; coverage map (colors encode signal per palette). |
| `ZDMR Iquique.prj` | CRS text (WGS 84); pair with .pgw for GIS (QGIS, ArcGIS, etc.). |
