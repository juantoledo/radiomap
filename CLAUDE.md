...
6. **Deploy** — Push to `main` runs [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml): regenerates `data.js`, bumps `VERSION`, replaces `__VERSION__` in HTML. Conventional commits drive semver (`feat` → minor, breaking → major). To publish **an existing tag** to Pages (no version bump), run [`.github/workflows/deploy-tag.yml`](.github/workflows/deploy-tag.yml) manually (**Actions** → **Deploy tag to Pages** → **Run workflow**) and enter the tag (e.g. `v1.2.3`). [`.github/workflows/sync-redchile-nodes.yml`](.github/workflows/sync-redchile-nodes.yml) runs daily (and via manual dispatch): renders `redchile.org/#!/nodos`, has Claude propose `curated_stations.csv` updates for that network, validates structure with `scripts/ci/validate-csv.py`, and opens a PR (never pushes to `main` directly) — review bot PRs like any other CSV change.
